...
$
    M_X(t) = E[e^(t X)] = int_( - infinity )^( + infinity ) e^(t x) f_X(x) d x\
...
$
    phi_X (t) = E[e^(i t X)] = int_( - infty )^( + infty ) e^(i t x) f_X(x) d x
$

这里取 $Y_k=X_k-mu,E(Y_K)=0,D(Y_k)=sigma^2$ 考虑独立同分布的特征函数均为 $phi_Y(t)$ ，由卷积定理得
